...
2. SQL Editor → run, in order: `supabase/schema.sql`, `migration_arms.sql`,
   ...
   `migration_school_band.sql` (the school's own manually-assigned **HPA/LPA**
   band on each learner, for the determined-vs-not-determined comparison and the
   label-vs-performance mismatch view).
